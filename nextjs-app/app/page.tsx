...
export default function HomePage() {
  ...
  // Live movies state initialized with authentic 92 real movies!
  const [liveMovies, setLiveMovies] = useState<DisplayMovie[]>(FALLBACK_MOVIES);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (supabase) {
          ...
          if (!error && data && data.length > 0) {
            const mapped: DisplayMovie[] = data.map((rec: MovieRecord) => ({
              ...
              poster: rec.poster_url || 'https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=400',
              backdrop: rec.backdrop_url || rec.poster_url || undefined,
        ...
        // 3. Fallback to local /fallback_catalog.csv
        const localRes = await fetch('/fallback_catalog.csv');
        if (localRes.ok) {
  ...
  // Hero movie from user's live list (Moana / pinned / first movie)
  const heroMovie = useMemo(() => {
    if (allMoviesList.length > 0) {
      const moana = allMoviesList.find((m) => m.title.toLowerCase().includes('moana'));
      if (moana) return moana;
      const pinned = allMoviesList.find((m) => m.type === 'movie' && (m.pin === '1' || m.pin === 'true' || m.pin === 'pin'));
      if (pinned) return pinned;
      return allMoviesList.find((m) => m.type === 'movie') || allMoviesList[0];
    }
    return null;
  }, [allMoviesList]);

  const suggestionsList = useMemo(() => {
    if (allMoviesList.length === 0) return [];
    let list = allMoviesList.filter(
      (m) => m.category?.toLowerCase().includes('suggest') || m.trending === '1'
    );
    if (list.length === 0) {
      list = heroMovie ? allMoviesList.filter((m) => m.id !== heroMovie.id) : allMoviesList;
    }
  ...
  }, [allMoviesList, heroMovie, suggestionsFilter]);

  const latestMoviesList = useMemo(() => {
    const list = allMoviesList.filter((m) => m.type !== 'series' && !m.category?.toLowerCase().includes('series'));
  ...
  const latestSeriesList = useMemo(() => {
    const list = allMoviesList.filter((m) => m.type === 'series' || m.category?.toLowerCase().includes('series') || Boolean(m.episode));
  ...
  const renderCard = (movie: DisplayMovie, _showEps?: boolean) => {
